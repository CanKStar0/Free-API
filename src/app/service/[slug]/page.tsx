...
// 1. Static site generation for all 500+ APIs
...
// 2. Dynamic SEO Metadata with TR & EN Alternates & OpenGraph
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const pageTitle = `${api.name} Free API - Docs, Rate Limits & Code Snippets`;
  const pageDescription = `${api.name} (${api.categoryTitle}) free REST API service. ${descEn} Rate limit: ${api.rateLimit}. Get cURL, Python & JavaScript code snippets.`;
  ...
  return {
    ...
    alternates: {
      canonical: `${siteUrl}/service/${api.slug}`,
      languages: {
    ...
    openGraph: {
      title: `${api.name} - Free API Service | FreeAPI Directory`,
      description: pageDescription,
    ...
    twitter: {
      ...
      title: `${api.name} - Free API Service`,
      description: pageDescription,
    },
  };
}
...
export default async function ServicePage({ params }: PageProps) {
        ...
        mainEntity: [
          ...
          },
          {
            ...
            acceptedAnswer: {
              '@type': 'Answer',
              text: api.isNoAuth
                ? `No, ${api.name} is a Zero-Auth service. You can call endpoints without sign-up.`
  ...
  return (
    <>
      ...
      />
      <ServiceClient api={api} relatedApis={relatedApis} />
    </>
  );
}
